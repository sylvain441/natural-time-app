...
const props = defineProps({
  ...
  // Skip the built-in thumbnail facade and load the iframe straight away.
  // For callers that already own the click: the button they showed IS the
  // facade, so the "no iframe until someone asks for it" rule still holds.
  immediate: {
    type: Boolean,
    default: false
  }
...
const playerLoaded = ref(props.immediate)
