				...
				<footer class="z-20 text-center flex flex-col items-center p-6 pb-6 md:pb-12 w-full max-w-screen-sm">
					...
					<div v-if="!clockTimeTravelMode && clockShowTitle">
						<h1 
							...
							<span v-if="clockActivePanel === AVAILABLE_PANELS.locationPicker && contextStore.tempLongitude === null" class="px-3 py-1 bg-nt-yellow-light">
						...
						<!-- SUBTITLE -->
						 <div v-if="clockActivePanel === AVAILABLE_PANELS.locationPicker && contextStore.tempLongitude === null">
						...
						<div v-else-if="!clockTutorialMode && latitude && longitude">
							<h2 class="flex justify-evenly items-center text-slate-400 font-medium text-sm md:text-xl space-x-4">
								<span class="font-extrabold text-lg bg-black text-nt-yellow-light font-mono px-2 py-0 mb-3">
					...
					<div v-if="clockTutorialMode && clockTutorialCurrentStep < clockTutorialStepsTotal" 
						...
						<arrowsIcon 
							...
							<!-- Tutorial step -->
						<span v-if="clockTutorialMode" class="text-md text-slate-600">Tutoriel <span class="">{{ String(clockTutorialCurrentStep + 1).padStart(2, '0') }}/{{ String(clockTutorialStepsTotal + 1).padStart(2, '0') }}</span></span>
